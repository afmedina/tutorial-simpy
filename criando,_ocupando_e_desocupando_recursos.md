...
* ``` res.queue```: quais entidades (no caso, requisições) estão em fila no momento. O **número** de entidades em fila do recurso é obtido diretamento com o comando len(res.queue);
* ```res.users```: quais entidades (no caso, requisições) estão em atendimento no momento. O **número** de entidades em processo do recurso é obtido diretamento com o comando len(res.users).
