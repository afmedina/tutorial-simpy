...
Para garantir que um novo ciclo de abertura e fechamento se repita \(dentro do laço infinito criado - `while True`\), deve-se criar um novo evento e isso está garantido no início do laço na linha:
...
Outra possibilidade com os eventos é aguardar até que dois ou mais deles ocorram para continuar algum processo. O SimPy possui duas opções bem interessantes para isso:
...
No nosso exemplo, vamos sortear o tempo de corrida de cada bicho e identificar quem foi o vencedor. A seguir,  criaremos dois eventos que representam a corrida de cada bicho:
...
Na função `corrida`, criamos portanto os eventos `lebreEvent` e `tartarugaEvent`, mas atenção: **os eventos foram criados, mas não foram executados**. Como não existe um `yield` aplicado aos eventos, eles estão apenas criados na memória do Python, e esperando o momento de serem executados no SimPy.
...
Antes de avançar - e com o intuito de facilitar o aprendizagem do _lebrístico_ leitor - vamos acrescentar ao código uma função para imprimir o status de cada evento dentro de uma lista e enventos:
